...
# krneki __________________________________________________________________________
...
for edge in G.edges():
    key1, key2 = edge
    circle1 = circlepack_layout_sim[key1]
    circle2 = circlepack_layout_sim[key2]
    # Compute midpoint
    mid_x = (circle1.center.real + circle2.center.real) / 2
    mid_y = (circle1.center.imag + circle2.center.imag) / 2
    mid_point = complex(mid_x, mid_y)
    # Create a small circle at the midpoint
    intermediate_circle = SimCircle(mid_point, radius=0.1)  # Adjust radius as needed
    intermediate_circles.append(intermediate_circle)

def plot_circles_with_intermediates(circlepack_layout_sim, intermediate_circles, with_labels=True, with_title=True):
    ...
    for circle in intermediate_circles:
        center = circle.center
        radius = circle.radius
        circle_artist = plt.Circle((center.real, center.imag), radius, fill=True, color='green', alpha=0.5)
        ax.add_artist(circle_artist)
    ax.set_aspect('equal', 'box')
    if with_title:
        ax.set_title('Knot Diagram with Intermediate Circles')
    # Adjust plot limits
    all_centers = np.array([circle.center for circle in circlepack_layout_sim.values()] +
                           [circle.center for circle in intermediate_circles])
    min_x, max_x = np.min(all_centers.real), np.max(all_centers.real)
    min_y, max_y = np.min(all_centers.imag), np.max(all_centers.imag)
    padding = 1.0
    plt.xlim(min_x - padding, max_x + padding)
...
plot_circles_with_intermediates(circlepack_layout_sim, intermediate_circles)
