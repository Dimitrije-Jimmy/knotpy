...
def plot_spline_with_overpass(circles_data):
    from scipy.interpolate import make_interp_spline
    import numpy as np
    import matplotlib.pyplot as plt
    ...
    # Find the parameter t where the overpass occurs
    # Assuming the overpass is at the last point returning to 'a'
    overpass_start_idx = np.searchsorted(t_new, t[-2])
    overpass_end_idx = len(t_new) - 1

    # Plotting
    fig, ax = plt.subplots()

    # Plot the initial spline segment (up to the overpass start)
    ax.plot(x_smooth[:overpass_start_idx], y_smooth[:overpass_start_idx], 'r-', linewidth=2, zorder=1)

    # Plot the overpass segment on top
    ax.plot(x_smooth[overpass_start_idx - 1:], y_smooth[overpass_start_idx - 1:], 'r-', linewidth=2, zorder=3)

    # Plot the circles and their centers
    for center, radius in circles_data:
        circle = plt.Circle((center.real, center.imag), radius, fill=False, edgecolor='blue', zorder=2)
        ax.add_artist(circle)
        # Plot the center
        ax.plot(center.real, center.imag, 'ro', markersize=5, zorder=4)
    ...
    plt.ylim(min_y - padding, max_y + padding)
    plt.grid(True)
    plt.title('Spline with Overpass at Crossing Point')
    plt.show()

def plot_knot_with_bezier_and_gaps(circles_data):
    import numpy as np
    import matplotlib.pyplot as plt
    from scipy.interpolate import splprep, splev

    # Extract centers from circles_data
    centers = np.array([complex(c.real, c.imag) for c, _ in circles_data])
    x_points = centers.real
    y_points = centers.imag
    ...
    # Plot the entire spline
    ax.plot(x_smooth, y_smooth, 'r-', linewidth=2, zorder=1)

    # Identify crossing points (simplified)
    # For demonstration, let's assume crossing occurs at a specific parameter value
    crossing_t = 0.5  # Assume crossing occurs at t=0.5
    crossing_idx = np.searchsorted(unew, crossing_t)

    # Create a gap in the underpassing strand
    gap_size = 10  # Number of points to remove to create gap
    x_under = np.concatenate([x_smooth[:crossing_idx - gap_size//2], x_smooth[crossing_idx + gap_size//2:]])
    y_under = np.concatenate([y_smooth[:crossing_idx - gap_size//2], y_smooth[crossing_idx + gap_size//2:]])

    # Clear the plot and replot with gap
    ax.clear()
    ax.plot(x_under, y_under, 'r-', linewidth=2, zorder=1)

    # Plot the overpassing segment on top
    x_over = x_smooth[crossing_idx - gap_size:crossing_idx + gap_size]
    y_over = y_smooth[crossing_idx - gap_size:crossing_idx + gap_size]
    ax.plot(x_over, y_over, 'r-', linewidth=2, zorder=3)
    ...
    plt.ylim(min_y - padding, max_y + padding)
    plt.grid(True)
    plt.title('Knot Diagram with Bezier Curves and Underpass Gap')
    plt.show()
...
# Plot the spline with cumulative distance parameterization and overpass
plot_spline_with_overpass(circles_data)
# Define crossings (for demonstration purposes)
# You can adjust 'crossing_t' based on actual data
crossings = [{'t': 0.5, 'over': True}]  # Crossing occurs at t=0.5, overpassing

# Plot the knot diagram with Bezier curves and underpass gap
plot_knot_with_bezier_and_gaps(circles_data)
