...
def extract_sequence_from_dict(ret):
    # Step 1: Build the graph
    graph = defaultdict(list)  # Adjacency list
    edge_keys = {}  # Map edges to their keys

    # Collect nodes and edges
    for key in ret.keys():
        if isinstance(key, str):
            # Main node (e.g., 'a', 'b', 'c')
            pass  # Nodes will be collected from edges
            ...
            node1 = str(label1)[0]  # Extract node from first character
            node2 = str(label2)[0]
            # Add edge to the graph
            graph[node1].append((node2, key))
            graph[node2].append((node1, key))
            # Map the edge key to the node pair
            edge_keys[frozenset({node1, node2, key})] = key
        else:
            continue  # Unknown key type

    # Step 2: Find an Eulerian trail
    sequence = find_eulerian_trail(graph)

    if sequence is None:
        print("No Eulerian trail found in the graph.")
        return None

    # Step 3: Reconstruct the sequence with edges
    final_sequence = []
    for i in range(len(sequence) - 1):
        node = sequence[i]
        next_node = sequence[i + 1]
        # Find the edge connecting node and next_node
        for neighbor, edge_key in ret.get(node, []):
            if neighbor == next_node:
                final_sequence.append(node)
                final_sequence.append(edge_key)
                break
        else:
            # Edge not found in adjacency list
            # Try the graph
            for neighbor, edge_key in graph[node]:
                if neighbor == next_node:
                    final_sequence.append(node)
                    final_sequence.append(edge_key)
                    break
            else:
                print(f"No edge found between {node} and {next_node}.")
                return None
    final_sequence.append(sequence[-1])  # Add the last node

    return final_sequence
"""
# My own extract_sequence_from_dict
def extract_sequence_from_dict(ret):
    # Step 1: Build the graph
    graph = defaultdict(list)  # Adjacency list
    edge_keys = {}  # Map edges to their keys
...
def extract_sequence_from_dict(ret):
    ...
    for key in ret.keys():
        if isinstance(key, str):
            ...
            if len(labels) != 2:
                continue  # Skip invalid edges
            var1, var2 = labels
            # Extract node labels from variables
            node1_label = str(var1)[0]
            node2_label = str(var2)[0]
            # Add edge to the graph (undirected)
            graph[node1_label].append((node2_label, key))
            graph[node2_label].append((node1_label, key))
            # Map the edge key to the node pair
            edge_keys[frozenset({node1_label, node2_label, key})] = key
        else:
            continue  # Unknown key type

    # Proceed to find the Eulerian trail
    sequence = find_eulerian_trail(graph)
    ...
    # Reconstruct the sequence with edges
    final_sequence = []
    for i in range(len(sequence) - 1):
        node = sequence[i]
        next_node = sequence[i + 1]
        ...
        for neighbor, edge_key in graph[node]:
            if neighbor == next_node:
                # Remove this edge to prevent reusing it
                graph[node].remove((neighbor, edge_key))
                graph[neighbor].remove((node, edge_key))
                final_sequence.append(node)
                final_sequence.append(edge_key)
                break
        else:
...
def find_eulerian_trail(graph):
    # Copy the graph to modify it
    local_graph = {node: adj.copy() for node, adj in graph.items()}
    trail = []

    # Function to perform the Hierholzer's algorithm
    def visit(u):
        while local_graph[u]:
            v, edge_key = local_graph[u].pop()
            # Remove the edge from the neighbor's list
            local_graph[v].remove((u, edge_key))
            visit(v)
            trail.append((u, edge_key))
        # Append the node when backtracking

    # Find the starting node
    odd_degree_nodes = [node for node in local_graph if len(local_graph[node]) % 2 == 1]
    if len(odd_degree_nodes) in [0, 2]:
        start_node = odd_degree_nodes[0] if odd_degree_nodes else next(iter(local_graph))
    else:
    ...
    # Build the sequence from the trail
    sequence = []
    sequence.append(start_node)
    for node, edge_key in reversed(trail):
        sequence.append(edge_key)
        sequence.append(node)
    return sequence

#sequence = extract_sequence_from_dict(ret)
