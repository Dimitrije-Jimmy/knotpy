...
def build_graph(ret):
    ...
    for key, circle in ret.items():
        if isinstance(key, frozenset):
            ...
            graph[node_from].append((node_to, key))
            graph[node_to].append((node_from, key))
            circles[key] = circle
...
def find_eulerian_trail(graph, start_node):
    trail = []
    stack = [start_node]
    local_graph = {node: edges.copy() for node, edges in graph.items()}

    while stack:
        node = stack[-1]
        if local_graph[node]:
            neighbor, conn_key = local_graph[node].pop()
            # Remove the edge in the neighbor's list
            local_graph[neighbor].remove((node, conn_key))
            stack.append(neighbor)
        else:
            trail.append(stack.pop())
    return trail[::-1]  # Reverse to get the correct order


def reconstruct_sequence(trail, graph):
    sequence = []
    for i in range(len(trail) - 1):
        node = trail[i]
        next_node = trail[i + 1]
        # Find the connection between node and next_node
        for neighbor, conn_key in graph[node]:
            if neighbor == next_node:
                sequence.append(node)
                sequence.append(conn_key)
                break
    sequence.append(trail[-1])  # Add the last node
    return sequence
...
# Find the Eulerian trail starting from node 'a'
trail = find_eulerian_trail(graph, 'a')

# Reconstruct the sequence of nodes and connections
sequence = reconstruct_sequence(trail, graph)
